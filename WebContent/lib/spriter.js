...
Spriter.Actor = function(game, x, y, spriterProjectKey, texutreKey, entityName) {
    ...
    this.offsize= new Phaser.Point(0,0);
    this.incam = true;
...
Spriter.Actor.prototype.preUpdate = function() {
	if(this.body){
	  this.body.height = this.height - this.body.offset.y;
	  this._bounds.x = this.body.x;
	  this._bounds.y = this.body.y;
	  this.incam = this.game.world.camera.view.intersects(this._bounds);
	  //this.body.width  = this.width;// - this.offsize.width;
	  if(!this.incam)return;
	}
